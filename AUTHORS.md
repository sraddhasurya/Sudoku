...
GenAI Usage:
We used ChatGPT primarily as a debugging and learning tool. It helped us interpret OCaml/dune errors and Yojson parse exceptions while building `Sudoku.load_grid` and `format_grid`, and it guided us toward the right checks when building validation helpers (`contains_all_digits_1_to_9`, `is_valid_sudoku`). It also helped us come up with useful test cases to add to verify that our logic is working thus far.
